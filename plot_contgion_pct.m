 %same ss contagion plot but in percentage of total spread instead of raw
 %bps
...
load('twotreesdecompose.mat');
 decompose_pct=decompose_stochastic./decompose_spread;
...
subplot(3,3,1)
  ...
  plot_c(s,decompose_pct,data_lev,data_mat,ylim_max)
...
subplot(3,3,9)
  ...
  xlabel('Share of tree A');
  ylim([-0.5,1.5]);
  function plot_c(s,decompose_pct,data_lev,data_mat,ylim_max)
  span = 10;
  series = [zeros(1,5)'; max(0,decompose_pct(6:end,data_mat,data_lev))];
...
%   y2a=decompose_stochastic(high,data_mat,data_lev)';
  ...
  xa=s;
  y1a=zeros(1,length(xa));
  y2a=max(0,decompose_pct(:,data_mat,data_lev)');
